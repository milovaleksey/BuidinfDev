-- ДЕМО-ДАННЫЕ: 5-этажное здание с помещениями и устройствами
...
BEGIN
    ...
    -- Холл
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor1_id, 'Холл', 'COMMON', 80.0, 'Главный вход и холл здания', NOW(), NOW())
    ...
    -- Ресепшн
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor1_id, 'Ресепшн', 'OFFICE', 25.0, 'Стойка регистрации посетителей', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Переговорная 101
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor1_id, 'Переговорная 101', 'MEETING', 20.0, 'Малая переговорная на 6 человек', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Переговорная 102
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor1_id, 'Переговорная 102', 'MEETING', 35.0, 'Большая переговорная на 12 человек', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Офис 201
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor2_id, 'Офис 201 - Бухгалтерия', 'OFFICE', 45.0, 'Отдел бухгалтерии, 8 рабочих мест', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Офис 202
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor2_id, 'Офис 202 - HR', 'OFFICE', 30.0, 'Отдел кадров, 5 рабочих мест', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Кухня-столовая
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor2_id, 'Кухня-столовая', 'COMMON', 40.0, 'Комната отдыха и приема пищи', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Офис 301
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor3_id, 'Офис 301 - Продажи', 'OFFICE', 60.0, 'Отдел продаж, 12 рабочих мест', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Офис 302
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor3_id, 'Офис 302 - Маркетинг', 'OFFICE', 50.0, 'Отдел маркетинга, 10 рабочих мест', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Офис 401 - IT
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor4_id, 'Офис 401 - IT отдел', 'OFFICE', 55.0, 'IT-отдел, 10 рабочих мест', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Серверная
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor4_id, 'Серверная', 'SERVER', 30.0, 'Серверная комната с системой охлаждения', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Кабинет директора
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor5_id, 'Кабинет директора', 'OFFICE', 40.0, 'Кабинет генерального директора', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Конференц-зал
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor5_id, 'Конференц-зал', 'MEETING', 80.0, 'Большой конференц-зал на 30 человек', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    -- Переговорная 501
    INSERT INTO rooms (floor_id, name, type, area_sqm, description, created_at, updated_at)
    VALUES (floor5_id, 'Переговорная 501', 'MEETING', 25.0, 'VIP переговорная', NOW(), NOW())
    RETURNING id INTO room_id;
    ...
    VALUES 
        ...
        (room_id, 'Кондиционер 501', 'HVAC', 'Сплит-система премиум-класса', 'hvac/floor5/meeting501/state', 'hvac/floor5/meeting501/set', 'ONLINE', NOW(), NOW());
    
    RAISE NOTICE '✅ Демо-данные успешно созданы!';
    RAISE NOTICE 'Здание: %', building_id;
...
\echo ''
\echo '📐 ЭТАЖИ:'
SELECT f.id, b.name as building, f.name as floor, f.level 
FROM floors f 
JOIN buildings b ON f.building_id = b.id 
ORDER BY b.id, f.level;

\echo ''
\echo '🚪 ПОМЕЩЕНИЯ ПО ЭТАЖАМ:'
SELECT 
    f.name as floor, 
...
\echo ''
\echo '✅ Готово! Демо-данные загружены в базу.'
